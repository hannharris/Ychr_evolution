...
gc <- read.delim(paste0(myPath, "/tables/GC_1030.csv", sep = ",")[2:5]
...
perc <- read.delim(paste0(myPath, "/tables/percent_alignment_1030.csv", sep = ",")[2:4]
...
pdf(file= paste0(myPath, "/1D_GC_scatter.pdf"), width=3, height=1.5, colormodel = "rgb")
...
pdf(file=paste0(myPath, "/1C_percentalign.pdf"), width=3, height=4, colormodel = "rgb")
violin_plot + 
...
tableb <- read.delim(paste0(myPath, '/tables/fig1b.txt'), check.names = FALSE)
...
table_gt
gt::gtsave(table_gt, "my_table.docx") #can drag into illustrator



#graph_rolling

perc_rolling <- read.delim(paste0(myPath, "/tables/perc_rolling_1208.csv"), sep = ",")

pdf(file=paste0(myPath, "/S3_supprolling.pdf"), width=8, height=6, colormodel = "rgb")
...
cpg <- read.delim(paste0(myPath, "/tables/CpG_1127.csv"), sep = ",")
...
pdf(file=paste0(myPath, "/S6_CpG.pdf"), width=4, height=4, colormodel = "rgb")
...
pdf(file=paste0(myPath, "/figS6_scatter.pdf"), width=4, height=4, colormodel = "rgb")
...
hdb <- read.delim(paste0(myPath, '/tables/percent_alignment_1211_dogbull.csv', sep = ","))[2:5]
...
pdf(file=paste0(myPath, "/S4_percentIDdogbull.pdf"), width=4, height=4, colormodel = "rgb")
violin_plot + 
...
gc_p <- read.delim(paste0(myPath, '/tables/percent_GC_1211_dogbull.csv'), sep = ",")[,2:5]
...
pdf(file=paste0(myPath, "/S5_dog_GC.pdf"), width=5, height=2.25, colormodel = "rgb")
...
pdf(file=paste0(myPath, "/S5_bull_GC.pdf"), width=5, height=2.25, colormodel = "rgb")
...
gc_chicken <- read.delim(paste0(myPath, "/tables/chicken_gc.txt"), sep = ",")
...
pdf(file=paste0(myPath, "/1_chicken_GC.pdf"), width=2.25, height=2.25, colormodel = "rgb")
...
gc_chicken <- read.delim(paste0(myPath, "/tables/opossum_gc.txt"), sep = ",")
g_p <- g_p %>% rename('gene' = 'gene.x')
...
pdf(file=paste0(myPath, "/1_opossum_GC.pdf"), width=2.25, height=2.25, colormodel = "rgb")
rbound %>% ggplot(aes(x=gc, y=GC_perc, color = xg)) +
  geom_point(stroke = 0) +
  theme_pubr() +
  #geom_text_repel(aes(label = gene)) +
...
pdf(file=paste0(myPath, "/2_opossum_GC.pdf"), width=2.25, height=2.25, colormodel = "rgb")
