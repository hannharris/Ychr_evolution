...
#not used --> calculate %log2(RNA/DNA) >0 in promoter region
```{r}

# full_fib_50_dist_5hun <- full_fib_50_dist %>% filter(distance <= 500 & distance >= -500)
# full_fib_50_dist_5hun$isunder0 <- ifelse(full_fib_50_dist_5hun$distance <= 0, "yes", "no" )
# 
# lcl_50_dist_5hun <- lcl_50_dist %>% filter(distance <= 500 & distance >= -500)
# lcl_50_dist_5hun$isunder0 <- ifelse(lcl_50_dist_5hun$distance <= 0, "yes", "no" )
# 
# 
# 
# lcl_50_dist_5hun %<>%
#   group_by(gene, isunder0) %>%
#   mutate(sum_over0 = sum(log2 >0)) %>% ungroup()
# 
...
# 
# 
# lcl_50_dist_5hun_g <- (lcl_50_dist_5hun %>% select(c("gene", "sum_over0", "isunder0")) %>% distinct()) %>% filter(gene != "ACTB" & gene != "CMV" & gene != "EEF1A1") 
# 
# lcl_50_dist_5hun_g$sum_bps <- lcl_50_dist_5hun_g$sum_over0 * 50
# 
# lcl_50_dist_5hun_g %<>% pivot_wider(id_cols = 'gene', names_from = 'isunder0', values_from = 'sum_bps')
# 
