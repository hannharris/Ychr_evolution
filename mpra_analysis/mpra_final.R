...
start_sites <- read.delim(paste0(myPath, "/tables/starts.txt"))
start_sites <- rbind(start_sites, data.frame("gene" = c("EEF1A1", "ACTB", "CMV"), "start" = c(73521032, 5530709, 1))) #positive ctls 

fib_counts_1 <- read.delim(paste0(myPath, "/tables/Fib_7169_1_counts.tsv"))
fib_counts_2 <- read.delim(paste0(myPath, "/tables/Fib7276_2_counts.tsv"))
...
gene_pairs <- read.delim(paste0(myPath, "/tables/g_p.txt")) 
...
metadata <- read.delim(paste0(myPath, "/tables/metadata_0119.txt"))  
...
pdf(file=paste0(myPath, "/S10_lib_content.pdf"))  
...
pdf(file=paste0(myPath, "/S10_corr_libs.pdf"), width=4, height=4, colormodel = "rgb") 
...
pdf(file=paste0(myPath, "/4C_corr_full_fib_lcl.pdf"), width=4, height=4, colormodel = "rgb")
